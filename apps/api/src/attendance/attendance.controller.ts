...
import type { AttendanceReport, UploadResponse, DailyAttendance } from '@attendance/shared';
...
export class AttendanceController {
  ...
  async generatePdfReport(
    ...
    try {
      // Generate HTML first
      const { html, filename } = this.reportTemplateService.generateHtmlReport(dto);
  ...
  @Post('upload')
  @ApiOperation({ summary: 'Upload and process attendance and user data files' })
  ...
  async uploadFile(
    @UploadedFiles() files: { attendanceFile?: Express.Multer.File[], userFile?: Express.Multer.File[] },
    ...
    try {
      ...
      const fileExt = '.' + attendanceFile.originalname.split('.').pop()?.toLowerCase();
